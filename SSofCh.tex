...
\begin{thm}{\cite[Prop.\ 1.2.2.14]{Lur17}}\label{th:converge}
    ...
    structure is compatible with sequential colimits. Let $F$ be a bounded below filtered object in $\cC$ (see \cref{def:Filb}). Then the spectral sequence $\{ E_r^{n,p}, d_r^{n,p} \}$ (see \cref{def:SS_Fil}) converges to the homotopy groups of the colimit of $F$:
    \begin{equation*}
...
\begin{equation}\label{eq:res_from_below}
    (i_{\geq a})^* \colon \Fun(\Ch, \cC) \to \Fun(\Ch_{\geq a}, \cC) \eqcolon \Ch_{\geq a}(\cC)
\end{equation}

admits the following right adjoint:
\begin{equation*}
    (i_{\geq a})_* \colon \Ch_{\geq a}(\Sp) \to \Ch(\Sp)
\end{equation*}
which given by right padding with zeros:
\begin{equation}\label{eq:right_padding}
...
\begin{equation}\label{eq:res_from_above}
...
\begin{defn}
    ...
    \begin{equation*}
        \nonFil_{[a,b]}(\Sp) \coloneqq \Fun([a,b],\Sp)
    \end{equation*}
\end{defn}
...
\begin{prop}\label{lem:ChSur}
    ...
    \[
        \bbS^{n-p}_p \to K_{[p,p-r+1]}
    \]
    such that the restriction
    \[
        \bbS^{n-p}_p \to K_{[p,p-r+1]} \to i_{\geq p}^* K_{[p,p-r+1]} = K_p
    \]
    is $x$.
\end{prop}

We depict such morphism as
\begin{equation*}
    \xymatrix{
...
\begin{proof}
    ...
    \begin{equation}\label{eq:postJ_RHS}
...
\begin{defn}\label{def:cond_conv}
    A spectral sequence $\{ E_r^{n,p}, d_r^{n,p} \}$ associated with a filtered object $F$ converges conditionally to the homotopy groups of $\colim_p F_p$ if $F$ is exhaustive and complete. Specifically, following Boardman \cite[Def.\ 5.10]{Boa99}, completeness requires that both the limit and the derived limit over the filtering vanish. In the $\infty$-categorical setting, this is equivalent to the single condition $\lim_p F_p \simeq \zr$.
\end{defn}

\begin{rem}
    For any coherent chain complex $K$, the associated filtered object $F = \cI K$ is complete by construction.
\end{rem}

\begin{cor}\label{cor:strong_cond_conv}
    Let $K \in \Ch(\Sp)$ be a coherent chain complex and $F = \cI K$ its equivalent filtered spectrum.
    \begin{enumerate}
        \item The associated spectral sequence converges conditionally to $\pi_* \colim_p F_p$.
        \item If $K$ is bounded below, then the spectral sequence converges strongly to $\pi_* \colim_p F_p$.
    \end{enumerate}
\end{cor}
\begin{proof}
    For the first statement, since $F$ is complete ($\lim_p F_p \simeq \zr$), the Milnor exact sequence applied to the sequential limit of $F_p$ implies that the $\lim^1$ term also vanishes. This satisfies the precise criteria for conditional convergence \cite[Def.\ 5.10]{Boa99}.
    For the second statement, if $K$ is bounded below, then $F$ is a bounded below filtered object. By \cref{th:converge}, this bounded below condition promotes conditional convergence to strong convergence.
\end{proof}
...
\subsubsection*{Chain-like description of the differential}
...
\begin{proof}
    ...
    \begin{equation*}
        \Map(\bbS_{p}^{-p}, K) \simeq \Map(\S^{-1} T \bbS_{p}^{-p},K) \simeq \Map(\bbS_{p+1}^{-(p+1)},K)
    \end{equation*}
...
\begin{exmp*}
    To get the direction right, note that $T \bbS_0 = \bbS_1$ and get that:
    \[
        \Map(\bbS_1, \bbS_0) \simeq \Map(\S^{-1} T \bbS^1_0, \bbS_0) \simeq \Map(\bbS^1_0, \bbS_0).
    \]
    So shifting the sourse to the right shifts the homotopical degree of the source one up.
\end{exmp*}
...
\begin{lem}\label{lem:zero_dr_extension}
    Let $K \in \Ch(\Sp)$, let $F=\cI K$, and let $r\geq 1$. Suppose that
    $x\colon \bbS^{n-p}\to K_p$ represents an element of
    $\cE_r^{n,p}\subseteq E_1^{n,p}$, and write $[x]_r\in E_r^{n,p}$ for
    its class. For an extension
    $\widetilde x\colon \bbS^{n-p}_p\to K_{[p,p-r+1]}$, write
    $\partial_r(\widetilde x)\in\cE_r^{n-1,p-r}\subseteq E_1^{n-1,p-r}$
    for the representative of $d_r^{n,p}([x]_r)$ given by
    \cref{lem:Ch_diff}. The following
    conditions are equivalent:
    \begin{enumerate}
        \item $d_r^{n,p}([x]_r)=0$ in $E_r^{n-1,p-r}$;
        \item there is an extension $\widetilde x$ of $x$ for which
              $\partial_r(\widetilde x)=0$ in $E_1^{n-1,p-r}$;
        \item $x$ extends to a map of coherent chain complexes
              $\bbS^{n-p}_p\to K_{[p,p-r]}$.
    \end{enumerate}
\end{lem}
\begin{proof}
    By \cref{lem:FpFn_is_rep,lem:ChSur}, an extension $\widetilde x$
    corresponds to a lift $\alpha\in\pi_n(F_p/F_{p-r})$ of $x$. Its
    obstruction $u=\partial_r(\widetilde x)$ is the image of $\alpha$
    under the connecting homomorphism of the cofiber sequence
    \[
        F_{p-r}/F_{p-r-1}\longrightarrow F_p/F_{p-r-1}
        \longrightarrow F_p/F_{p-r}.
    \]
    Thus $u\in\pi_{n-1}(F_{p-r}/F_{p-r-1})=E_1^{n-1,p-r}$.
    By \cref{def:SS_Fil,lem:Ch_diff}, the image of $u$ in
    $\pi_{n-1}(F_{p-1}/F_{p-r-1})$ is $d_r^{n,p}([x]_r)$, viewed in
    this group via the inclusion in \cref{eq:Er_Fil}.

    Suppose this image vanishes. Exactness for the cofiber sequence
    \[
        F_{p-r}/F_{p-r-1}\longrightarrow F_{p-1}/F_{p-r-1}
        \longrightarrow F_{p-1}/F_{p-r}
    \]
    identifies the target boundary subgroup, in the usual notation, as
    \[
        \begin{aligned}
            B_r^{n-1,p-r}
            &=\operatorname{im}\bigl(\pi_n(F_{p-1}/F_{p-r})
              \xrightarrow{\partial} E_1^{n-1,p-r}\bigr)\\
            &=\ker\bigl(E_1^{n-1,p-r}\to
              \pi_{n-1}(F_{p-1}/F_{p-r-1})\bigr).
        \end{aligned}
    \]
    It therefore gives $\beta\in\pi_n(F_{p-1}/F_{p-r})$ whose
    boundary is $u$.
    Let $j$ be the map from $\pi_n(F_{p-1}/F_{p-r})$ to
    $\pi_n(F_p/F_{p-r})$ induced by $F_{p-1}\to F_p$, and set
    $\alpha'=\alpha-j(\beta)$. The image of
    $\alpha'$ in $\pi_n(F_p/F_{p-1})$ is still $x$. By naturality of
    the connecting homomorphisms, its obstruction is
    $u-\partial(j(\beta))=u-u=0$. Hence $\alpha'$ gives the
    extension in (2). Conversely, (2) immediately implies (1).

    Exactness for the first cofiber sequence shows that an extension
    has zero obstruction precisely when its class $\alpha'$ lifts to
    $\pi_n(F_p/F_{p-r-1})$. By \cref{lem:ChSur}, this is precisely an
    extension of $x$ to $K_{[p,p-r]}$, proving (2)$\Leftrightarrow$(3).
    In the same notation, $\cE_r$ is $Z_r$. Exactness of
    $F_{p-1}/F_{p-r}\to F_p/F_{p-r}\to F_p/F_{p-1}$ shows that any
    two extensions of $x$ differ by some $j(\beta)$. Thus varying
    the extension changes its raw obstruction by exactly the subgroup
    $B_r$ displayed above.
\end{proof}
