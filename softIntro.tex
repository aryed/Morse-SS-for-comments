...
\end{equation*}
See \cref{sec:FFCat} for definitions and details. Another building block in the theory are \emph{flow modules}. Given a flow category $\fC$, a right flow module\todo{add emph to the "right flow module"} $W$ over $\fC$ (see \cref{def:RightFlowModule} for the formal definition) consists of a collection of manifolds $\{W_x\}_{x\in \fC}$, together with corner structure maps \begin{equation*}
...
\begin{equation*}
    \norm{x} - \dim (V_x) = \norm{x} - (\norm{x} - \norm{x'} - 1) = \norm{x'} + 1
\end{equation*}
and we denote is just by $\flowM_{-,x'}$ \todo{I think that this "just by" needs some grammer fixing}.
...
\begin{minipage}{.61\textwidth}
  \centering
  \adjustbox{width=\linewidth}{\input{fig/k-boundary Alt.tex}}
  \captionof{figure}{
  the \cref{fig:ffCat} (from $*$ to $z$) and the\\object $*$ construct the punctured cube $D_{z}$
  }\todo{this description need math adn grammer corerctions}
...
\begin{restatable}{thmA}{ThA}\label{SSofStrFFCat}
    ...
    \begin{equation*}
        d_{r+1}(W_p) = \{\flowM_{-,y} \circ W\}_{y \in \fC_{p-(r+1)}} \in \bigoplus_{y \in \fC_{p-(r+1)}} \Omega^{\cX}_{n-p+r} = \pg{E}{r+1}{n-1}{p-(r+1)}
...
\]
from a knot diagram. This leads to the calculation of the corresponding homotopy (see \cite{lobb2020khovanov, lipshitz2014steenrod}).
