...
\begin{restatable}{thmA}{ThA}\label{SSofStrFFCat}
    ...
    \end{equation*}
    survives to $\pg{E}{r+1}{n}{p}$ if and only if the collection $W_p$ extends to a right flow module over $\fC_{[p,p-r]}$ (see \cref{def:ModuleSurvives}). The differential is represented geometrically by the Toda manifolds:
    \begin{equation*}
...
\begin{restatable}{thmA}{ThB}\label{SSofCh}
    Let $K \in \Ch(\Sp)$ be a coherent chain complex in Spectra\todo{READ carefully about what condistions we need to get convergences}. Then there is a spectral sequence:
    \begin{equation*}
        E_1^{n,p} = \pi_{n - p} K_p,
    \end{equation*}
    which converges to
