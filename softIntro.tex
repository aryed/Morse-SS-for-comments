...
Specifically, for each $z \in \fC_{p-(r+1)}$ we can take the left flow module $\flowM_{-,z}$ and the right flow module $W$ to construct the Toda manifold $\flowM_{-,z} \circ W \in \Omega^{\cX}_{n-p+r}$ and by taking the union we get an element
\begin{equation*}
    \{\flowM_{-,z} \circ W\}_{z \in \fC_{p-(r+1)}} \in \bigoplus_{z \in \fC_{p-(r+1)}} \Omega^{\cX}_{n-p+r}
\end{equation*}
...
\begin{minipage}{.61\textwidth}
  ...
  \captionof{figure}{
  the  \cref{fig:ffCat} (from $x$ to $x_2$) and the\\object $x_1$ construct the punctured cube $D_{x_1}$
  }
