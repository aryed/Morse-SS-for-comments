...
\end{equation*}
See \cref{sec:FFCat} for definitions and details. Another building block in the theory are \emph{flow modules}. Given a flow category $\fC$, a \emph{right flow module} $W$ over $\fC$ (see \cref{def:RightFlowModule} for the formal definition) consists of a collection of manifolds $\{W_x\}_{x\in \fC}$, together with corner structure maps \begin{equation*}
...
\begin{equation*}
    [*[i],\fC].
\end{equation*}
Similarly, we can define left flow modules of degree $i$ over $\fC$ (see \cref{def:LeftFlowModule} for the formal definition) to be collections of manifolds $\{V_x\}_{x \in \fC}$, together with corner structure maps
\begin{equation*}
...
\begin{equation*}
    \norm{x} - \dim (V_x) = \norm{x} - (\norm{x} - \norm{x'} - 1) = \norm{x'} + 1
\end{equation*}
and we denote it simply by $\flowM_{-,x'}$.
...
An element $W_p = \{W_z\}_{\norm{z}=p} \in \pg{E}{1}{n}{p}$ survives to the $(r+1)$-th page if and only if it extends to a right flow module $W$ of degree $n$ over $\fC_{[p,p-r]}$ (see \cref{def:ModuleSurvives} for the formal definition).

To describe the differential, let $W,V$ be a right and left flow module over $\fC$ of degree $n$ and $p-r$ respectively (see \cref{def:RightFlowModule}, \cref{def:LeftFlowModule}). We define the punctured cube diagram
\begin{equation*}
...
\end{equation*}
and we will see later that the Toda manifolds indeed inherits manifold structure from $V,W$, and that they are closed, $(n-p+r)$-dimensional, $\cX$-structured.
...
\begin{minipage}{.61\textwidth}
  \centering
  \adjustbox{width=\linewidth}{\input{fig/k-boundary Alt}}
  \captionof{figure}{
  The punctured 3-cube diagram $D_{z}$ associated with the object $z = x_1$ of degree 1, constructed from the right flow module $W$ and the left flow module $\flowM_{-,z}$.
  }
...
\begin{minipage}{.37\textwidth}
  ...
  \captionof{figure}{
  We can see that $\colim D_{z}$ is a topological space that inherits a closed smooth manifold structure.
  }
...
\begin{restatable}{thmA}{ThA}\label{SSofStrFFCat}
    ...
    \end{equation*}
    which conditionally converges to the homotopy groups of the realization:\todo{resolve: by Lurie we ahve that the S.S convernce to the homotopy groups of the colim of the filtered object. We never justified why this colim is $\norm{\fC}$}
    \begin{equation*}
        \rnp{E} \implies \pi_n \norm{\fC}.
    \end{equation*}
    An element
    \begin{equation*}
        W_p=\{W_z\}_{\norm{z}=p} \in \pg{E}{1}{n}{p}
    \end{equation*}
    survives to $\pg{E}{r+1}{n}{p}$ if and only if the collection $W_p$ extends to a right flow module over $\fC_{[p,p-r]}$ (see \cref{def:ModuleSurvives}\todo{move this ref after the thm?}). The differential is represented geometrically by the Toda manifolds:
    \begin{equation*}
        d_{r+1}(W_p) = \bigcup_{y \in \fC_{p-(r+1)}} (\flowM_{-,y} \circ W) \in \bigoplus_{y \in \fC_{p-(r+1)}} \Omega^{\cX}_{n-p+r} = \pg{E}{r+1}{n-1}{p-(r+1)}
    \end{equation*}
\end{restatable}
...
\begin{restatable}{thmA}{ThB}\label{SSofCh}
    ...
    Moreover, let $x: \mathbb{S}^{n - p} \to K_p$ represent an element of $\pg{E}{1}{n}{p}$. Then $x$ survives to $\pg{E}{r+1}{n}{p}$ if and only if there exists a chain complex map:
...
\begin{rem}\label{rem:convergence}
    If the flow category $\fC$ (or the chain complex $K$) is bounded from below, the conditional convergence in \cref{SSofStrFFCat} and \cref{SSofCh} upgrades to strong convergence (referred to simply as convergence in \cite[Prop.\ 1.2.2.14]{Lur17}). In general, the stable filtered object associated with a chain complex is complete, which allows us to use the Milnor exact sequence to satisfy Boardman's completeness criteria \cite[Def.\ 5.10]{Boa99} and guarantee conditional convergence even when unbounded (see \cref{sec:SSofCh} for details).
\end{rem}

\subsection*{Relation to other work}
This paper contributes to the broader project of providing a geometric model for spectra (see \cite{abouzaid2024foundation}), building on the seminal work of Cohen, Jones, and Segal \cite{CJS95} who first realized Morse and Floer data as stable homotopy types.

In low-dimensional topology, flow categories are used to lift Khovanov homology to a stable homotopy type \cite{LS14}, where Pontryagin-Thom gluing of $1$-dimensional moduli spaces computes the secondary cohomology operation $\operatorname{Sq}^2$ \cite{lipshitz2014steenrod}. Our work generalizes this gluing to higher-dimensional flow manifolds with corners to geometrically model all higher spectral sequence differentials and Toda brackets.

In symplectic geometry, flow categories and modules arise in Lagrangian Floer theory to construct a spectral Fukaya category over the sphere spectrum \cite{porcelli2024bordism}, where successive truncations of flow modules act as obstructions to lifting quasi-isomorphisms. A natural direction for future work is generalizing our spectral sequence framework to equivariant flow categories, such as the Morse-Bott models of C{\^{o}}t{\'{e}}--Kartal \cite{cote2023equivariant} and the equivariant flow categories of Rezchikov \cite{rezchikov2024cyclotomic} used to construct cyclotomic structures on Floer spectra.

\textbf{Morphisms and invariance.} In \cite{abouzaid2024foundation}, the authors construct an $\infty$-category $\mathrm{Flow}^{\fr}$ (see also \cite{porcelli2024bordism} for the homotopy category) and show that it is equivalent to $\Sp$. The morphisms in this category are \emph{flow bimodules}. Given two Morse-Smale functions $f$ and $g$ on a compact manifold $M$, one obtains two framed flow categories, $\fC_f$ and $\fC_g$, and a flow bimodule between them, showing that the stable homotopy type is independent of the choice of Morse-Smale function. However, the different CW structures yield different spectral sequences, so the spectral sequence itself is not an invariant of the equivalence class in $\mathrm{Flow}^{\fr}$. To address this, we expect there is a construction analogous to that in \cite{abouzaid2024foundation} that defines another $\infty$-category with a modified class of morphisms. Specifically, for a 2-morphism $R$ between flow categories $\fC$ and $\sD$, we require that if $\|y\| > \|x\|$, then $R_{x,y} = \varnothing$. This construction would provide a flow-categorical model for the subcategory of $\{K_p\} \in \Ch(\Sp)$ satisfying $K_p = \bigoplus \mathbb{S}$. In this setup, the two Morse-Smale functions on a manifold $M$ produce distinct objects, and we can think of \cref{SSofStrFFCat} as a spectral sequence of a filtered flow category $F \in \Fil(\mathrm{Flow}^{\cX})$ induced by the index filtration.

\begin{rem}
    In this paper, we do not attempt to implement this full theoretical framework. The complete $\infty$-categorical structure is unnecessary for our purposes; instead, we only require a practical model for the composition of left and right flow modules to extract the geometric differentials of the spectral sequence.
\end{rem}

The interpretation of gluing manifolds as Toda brackets or Massey products in cobordism appears in \cite{alexander1972cobordism} (see also \cite{morisugi1983massey}), where the Massey product is defined via the gluing of manifolds with boundary.
