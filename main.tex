...
% === Page Layout ===
\usepackage[top = 1.5in, bottom = 1.5in, left=0.5in, right=1.5in, marginparwidth=1in]{geometry}
...
\todo{We change the intro to use "flow bimod", now we need to shcange the rest of the paper}
\input{softIntro}
...
% The incomplete duplicate composition draft formerly included here is archived at
% research/archived-drafts/CompInTop.tex. The active model is developed in
% ChModels.tex; see README.md and research/composition-rewrite-plan.md.

\section{Manifolds with Corners and Framed Flow Categories}
This section lays the geometric and categorical groundwork for our main constructions. We begin by introducing a diagrammatic framework. The fundamental geometric objects in this setting are manifolds with corners, which generalize smooth manifolds (see \cite{LS14} for a relevant treatment). These concepts are then used to define flow categories \cite{CJS95}, where the morphism spaces are manifolds with corners and composition is given by embedding into the corner strata. The subsequent subsections develop the precise definitions required for this approach.
\input{Mfld w. corners & ffcat}

\section{Chain complex of Flow Category}\label{sec:J}
...
\section{Composition through a Bounded Chain Complex}\label{sec:compOfFlow}
...
\input{StrFF-via spaces diagrams}
