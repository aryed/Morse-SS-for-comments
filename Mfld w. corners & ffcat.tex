...
\begin{defn}
    A \emph{neat embedding} of a flow category $\fC$ consists of a choice of integer $N$ and a family of neat embeddings of manifolds with corners,
    \begin{equation*}
...
\begin{rem}\label{rem:Cat_emb}\todo{do we need this? (wait for index bundle change first)}
...
\subsection{Flow Modules}\label{subsec:FlowModules}

\begin{defn}\label{def:RightFlowModule}
    Let $\fC$ be a flow category. A \textbf{right flow module of degree $n$} over $\fC$ consists of compact manifolds with faces $W_x$ of dimension $n-\norm{x}$, with $W_x=\varnothing$ if this dimension is negative, and specified face identifications
    \[
        \partial_i W_x
        =\bigsqcup_{\substack{y\in\fC\\\norm{y}=i}}
          W_y\times M_{y,x},
        \qquad \norm{x}<i\leq n.
    \]
    These faces cover the whole boundary of $W_x$. Their iterated intersections are the products
    \[
        W_{y_k}\times M_{y_k,y_{k-1}}\times\cdots\times M_{y_1,x}
    \]
    for strictly decreasing grading chains
    $\norm{y_k}>\cdots>\norm{y_1}>\norm{x}$, with the disjoint unions over objects of the specified grades understood. All ways of identifying an iterated face agree with flow composition and the module action, and there are no further boundary faces.
\end{defn}

\begin{defn}\label{def:LeftFlowModule}
    Let $\fC$ be a flow category. A \textbf{left flow module of degree $q$} over $\fC$ consists of compact manifolds with faces $V_y$ of dimension $\norm{y}-q$, empty in negative dimension, and specified face identifications
    \[
        \partial_i V_y
        =\bigsqcup_{\substack{x\in\fC\\\norm{x}=i}}
          M_{y,x}\times V_x,
        \qquad q\leq i<\norm{y}.
    \]
    These faces cover the whole boundary of $V_y$. Their higher intersections are exactly the iterated products of flow manifolds followed by a module manifold; the identifications agree with flow composition on every iterated face.
\end{defn}

\definitionchange{The right and left module definitions now require compact manifolds with faces and identify every boundary face with the stated products, including all higher intersections. The previous wording specified compatible boundary embeddings but did not explicitly exclude additional boundary faces. The framed definition below also makes compatibility of the embeddings explicit. These conditions are used to obtain a closed glued manifold; compare to the original definition. The intended dimensions and directions of the actions are unchanged.}

\begin{defn}\label{def:FramedFlowModule}
    Let $\fC$ be a neatly embedded, normally framed flow category. A \textbf{neatly embedded, normally framed right flow module} $W$ of degree $n$ over $\fC$ consists of a right flow module structure together with a choice of integer $N$, neat embeddings of manifolds with corners:
    \begin{equation*}
        e_x: W_x \hookrightarrow \bbR^{N(n+1-\norm{x})} \times \Hlf^{J_{n+1,x}}
    \end{equation*}
    which restrict on every face $W_y\times M_{y,x}$ to the product of the module and flow embeddings, under the fixed ambient coordinate identifications. These restrictions agree on all higher faces. The module also has stable normal framings $\varphi_x$ on $\nu e_x$, compatible on every face with the product framing:
    \begin{equation*}
        \partial_y \varphi_x = \varphi_y \oplus \varphi_{y,x}
    \end{equation*}
    where $\varphi_{y,x}$ is the framing on the morphism manifold $M_{y,x}$ of $\fC$.
\end{defn}

Similarly, we can define a neatly embedded, normally framed left flow module.

A usfeul constrution, which allow us to use the flow categories tools on flow modules is the following:

\begin{defn}\label{def:ConeFlowCategory}
    Let $W$ be a right flow module of degree $i$ over a flow category $\fC$, with flow manifolds $M_{y,x}$. We define the \textbf{Cone Flow Category} $\cone(W)$ as follows:
    \begin{itemize}
        \item The objects are $\mathrm{ob}(\cone(W)) = \mathrm{ob}(\fC) \sqcup \{z\}$, where $z$ is a new formal object with index $\norm{z} = i+1$.
        \item For $x, y \in \fC$, the morphism spaces remain identical: $(\cone(W))_{y,x} = M_{y,x}$.
        \item For $x \in \fC$, the morphism space from $z$ to $x$ is defined as the module manifold:\newline $(\cone(W))_{z,x} = W_x$. (Note that $\dim(W_x) = i - \norm{x} = \norm{z} - \norm{x} - 1$, which satisfies the dimension axiom for flow categories).
        \item The morphism spaces from $x$ to $z$ are empty.
    \end{itemize}

    The corner structure maps
    \begin{equation*}
        \coprod_{y \in \fC} (\cone(W))_{z,y} \times (\cone(W))_{y,x} \hookrightarrow \partial (\cone(W))_{z,x}
    \end{equation*}
    are given exactly by the module structure maps $\coprod_y W_y \times M_{y,x} \hookrightarrow \partial W_x$. If $\fC$ and $W$ are normally framed, these framings naturally endow the flow category $\cone(W)$ with a normal framing.

    Similarly, for a left flow module $V$ of degree $i$ over $\fC$, its \textbf{Cone Flow Category} $\cone(V)$ is defined analogously by adding a new object $z$ in degree $i-1$. The morphism spaces from $\fC$ to $z$ are given by $V$ and the corner structure maps are determined by the left module structure maps.
\end{defn}


\begin{defn}\label{def:ModuleSurvives}
    Let $\fC$ be a framed flow category, and let $n, p, r \in \bbZ$ with $r \geq 0$. Let $W_p = \{W_z\}_{\norm{z}=p}$ be a collection of closed framed manifolds representing an element in:
    \begin{equation*}
        \pg{E}{1}{n}{p} = \bigoplus_{\norm{z}=p} \Omega^{\fr}_{n-p}.
    \end{equation*}
    We say that the collection $W_p$ \textbf{extends to a right flow module of degree $n$ over $\fC_{[p,p-r]}$} if there exists a neatly embedded, normally framed right flow module $W$ of degree $n$ over $\fC_{[p,p-r]}$ whose restriction to the objects of degree $p$ satisfies $W_z = (W_p)_z$ for all $z \in \fC_p$.
\end{defn}
