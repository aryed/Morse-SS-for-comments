...
\begin{defn}\label{def:RightFlowModule}
    Let $\fC$ be a flow category. A \textbf{right flow module} $W$ of \textbf{degree} $n$ over $\fC$ consists of a collection of manifolds with corners $\{W_x\}_{x \in \fC}$ together with compatible corner structure maps (embeddings of codimension 1 boundary faces):
    \begin{equation*}
        W_y \times M_{y,x} \hookrightarrow W_x
    \end{equation*}
    for all $y, x \in \fC$, such that:
    \begin{equation*}
        \dim W_x = n - \norm{x}.
    \end{equation*}
\end{defn}
...
\begin{defn}\label{def:LeftFlowModule}
    Let $\fC$ be a flow category. A \textbf{left flow module} $V$ of \textbf{degree} $p$ over $\fC$ consists of a collection of manifolds with corners $\{V_x\}_{x \in \fC}$ together with compatible corner structure maps (embeddings):
    \begin{equation*}
...
\begin{defn}\label{def:FramedFlowModule}
    ...
    \end{equation*}
    and stable normal framings $\varphi_x$ on the normal bundles $\nu e_x$ that are compatible with the framing:
    \begin{equation*}
